...
#Load libraries ----
library(daltoolbox)
#library(daltoolboxdp) #Não necessita para métodos básicos
library(tspredit)
library(harbinger)
...
#RTAD muito lento e desnecessário para este artigo - Pode ser rodado para
#dissertação do Ricardo (se ele decidir com Ogasawara)
...
for (i in seq_along(series_ts)) {
  ...
  inicio <- 1L
  fim    <- 4032L
  #fim    <- 1538L #Menor série do grupo (rodar todas com o mesmo tamanho)
  if (is.null(n)) {
